...
const LeftSidebar = () => {
  ...
  const navigate = useNavigate();
  const { likeNotification } = useSelector((store) => store.realTimeNotification);
  const [isSidebarOpen, setSidebarOpen] = useState(false); // For mobile sidebar toggle
  ...
  return (
    <div>
      ...
      <div className="md:hidden fixed top-2 left-2 z-20">
        <button onClick={() => setSidebarOpen(!isSidebarOpen)} className="text-xl">
          ☰
        </button>
      </div>
      ...
      <div
        className={`fixed top-0 left-0 px-4 border-r border-gray-300 h-screen z-10 bg-white transition-transform duration-300 md:w-[16%] ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}
      >
        <div className="flex flex-col">
          <div className="flex items-center justify-evenly">
            <img src={LOGO} className="h-[35px]" />
          ...
          <div>
            {sidebarItems.map((item, index) => (
              ...
              >
                {item.icon}
                <span className="hidden md:inline">{item.text}</span> {/* Hide text on mobile */}
                {item.text === "Notifications" && likeNotification.length > 0 && (
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button size="icon" className="rounded-full h-5 w-5 absolute bottom-6 left-6 bg-red-600">
                          ...
                          likeNotification.map((notification) => (
                            <div key={notification.userId} className="my-5 flex items-center gap-2">
                              ...
                              {notification.type === "comment" ? (
                                <p className="text-sm">
                                  <span className="font-bold">{notification.userDetails?.name}</span> commented{" "}
                                  <span className="font-semibold">{`"${notification.comment?.text}"`}</span> on your post.
                                </p>
                              ) : (
                                <p className="text-sm">
                                  <span className="font-bold">{notification.userDetails?.name}</span> liked your post.
                                </p>
                              )}
                            </div>
                          ))
